...
public class When_using_Aes_with_unobtrusive_mode : NServiceBusAcceptanceTest
{
    ...
    public async Task Should_receive_decrypted_message()
    {
        var messageToSend = new MessageWithSecretData
        {
            ...
            SubProperty = new MySecretSubProperty { EncryptedSecret = "My sub secret" },
            CreditCards =
            [
        ...
        var context = await Scenario.Define<Context>()
            .WithEndpoint<Sender>(b => b.When(session => session.Send(messageToSend)))
            .WithEndpoint<Receiver>()
            .Done(c => c.Secret != null)
            .Run();
    ...
    static readonly Dictionary<string, byte[]> Keys = new() { { "1st", "gdDbqRpqdRbTs3mhdZh9qCaDaxJXl+e6"u8.ToArray() } };
