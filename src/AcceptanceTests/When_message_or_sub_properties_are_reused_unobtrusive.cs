...
public class When_message_or_sub_properties_are_reused_unobtrusive : NServiceBusAcceptanceTest
{
    [Test]
    public async Task Should_not_double_encrypt_properties()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            {
                Assert.That(messageToReuse.Secret, Is.EqualTo("betcha can't guess my secret"));
                Assert.That(messageToReuse.SubProperty.Secret, Is.EqualTo("My sub secret"));
            }
        });
    }
    ...
    public class Endpoint : EndpointConfigurationBuilder
    {
        public Endpoint()
        {
            ...
            EndpointSetup<DefaultServer>(config => config.EnableMessagePropertyEncryption(new AesEncryptionService("1st", keys), t => t.Name.StartsWith("Secret")));
        }
